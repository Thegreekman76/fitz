...
♠ **Jobs sin Celery** con `@cron("expr")` + `@background` + `spawn(fn_call)`. Sin broker externo (Redis/RabbitMQ no son requisito). Cron-only mode systemd-friendly. **v0.11.2 (iter2)**: kwargs opcionales `tz=` (IANA timezones), `retry={max, backoff, ...}` (3 backoffs), `catch_up=true|false` (missed runs al boot), `store=db` (persiste a `fitz_cron_jobs` + `fitz_cron_runs`). **v0.37.7**: los mismos kwargs de persistencia aplican a `@background` — `@background(store=db, catch_up=true, retry={...})` registra cada `spawn(...)` en `fitz_bg_jobs` (args serializados a JSON, primitivos + compuestos), así los jobs fire-and-forget sobreviven reinicios; `catch_up=true` marca huérfanos `failed` al boot. Paridad bit-a-bit `fitz run` ↔ `fitz build`. → [cap 30](https://thegreekman76.github.io/fitz/guide/#30-jobs-sin-celery).
