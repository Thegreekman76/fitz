...
> A modern compiled language for web services — HTTP, OpenAPI and a Postgres ORM in the core.
> Write your API in one file; compile it to a single dependency-free binary.

```fitz
// A complete REST CRUD API: HTTP + Postgres ORM + OpenAPI, zero dependencies.
...
```bash
fitz run app.fitz       # run it now (interpreter + static checker)
fitz build app.fitz     # native standalone binary (~5 MB)
./app                   # runs with no Fitz or Rust on the target
```

The Postgres ORM, OpenAPI docs at `/docs` and the single binary all come in the core — no libs, no config.
...
♠ **Jobs sin Celery** con `@cron("expr")` + `@background` + `spawn(fn_call)`. Sin broker externo (Redis/RabbitMQ no son requisito). Cron-only mode systemd-friendly. **v0.11.2 (iter2)**: kwargs opcionales `tz=` (IANA timezones), `retry={max, backoff, ...}` (3 backoffs), `catch_up=true|false` (missed runs al boot), `store=db` (persiste a `fitz_cron_jobs` + `fitz_cron_runs`). **v0.37.7**: los mismos kwargs de persistencia aplican a `@background` — `@background(store=db, catch_up=true, retry={...})` registra cada `spawn(...)` en `fitz_bg_jobs` (args serializados a JSON, primitivos + compuestos), así los jobs fire-and-forget sobreviven reinicios; `catch_up=true` marca huérfanos `failed` al boot. **v0.37.8**: cross-module en `fitz build` (port de B20) — el worker `@background(store=db)` puede vivir en un módulo importado (co-localizado con su `let db = ...`) con el `spawn(...)` en el main. Paridad bit-a-bit `fitz run` ↔ `fitz build`. → [cap 30](https://thegreekman76.github.io/fitz/guide/#30-jobs-sin-celery).

◐ **Frontend nativo `.fitzv` → WASM (Fase 11.7, v0.22.0)** — **el kanban como single-page app de WebAssembly**. Un single-file component `.fitzv` (estilo Vue/Svelte: `component X { state {...} event ...() {...} <template>...</template> <style scoped>...</style> }`) compila a un bundle WebAssembly standalone que corre 100% en el browser — sin server, sin runtime de JS framework. El **Board del kanban colaborativo** (nominal `Card` importado de un `.fitz` hermano, 6 helper fns clásicas transpiladas al crate WASM, `{#for c in cards_in(cards, "todo")}`, `.map`/`.filter` con closures, payload de click + form) compila desde UN `.fitzv` a ~57 KB raw / ~21.5 KB gzipped (`examples/view/kanban/`). Los mismos convenios `data-flv-*` sirven a los dos targets: **SSR** (server-rendered, sync multi-cliente por WebSocket, via [fitz-liveviews](https://github.com/Thegreekman76/fitz-liveviews)) y **client-WASM** (SPA offline). Composición completa: props (↓), event bubbling `<Child @event="h" />` **con payload** (↑, v0.23.0 — el parent lee `payload["k"]` para saber qué child disparó), slots — `<slot>fallback</slot>` default + **named slots** `<slot name="X" />` (v0.24.0, el parent llena cada región con `slot="X"` estilo Web Components), y **cross-file** — el `<Child />` puede vivir en un `.fitzv` separado importado con `from Card import Card` (v0.25.0, la surface entera cruza el borde de archivo), con **aliasing** (`import Card as Row`), **transitividad** (un grandchild en un archivo que el parent no importa se descubre solo) y **LSP cross-file** (editar el `.fitzv` en VSCode ya resuelve el child importado) — v0.26.0. 9 ejemplos runnable en `examples/view/` (counter → kanban → cross-file-child → cross-file-transitive), cada uno compilable a WASM real con `fitz build --bin <web> --target wasm-client`. **Ningún otro lenguaje** compila un SFC a un binario WASM standalone + SSR desde la misma fuente, con el mismo `type` compartido entre back y front. **Funciones de servidor `@rpc` (Fase 11.11, v0.30.0)** — una `@rpc async fn` en un `.fitz` classic (con acceso a DB/auth/secrets) es llamable **directo desde el `.fitzv`** como si fuera local: `let u = get_user(42).await?`. El compilador emite las **dos mitades** desde una sola declaración — un `POST /__rpc/<name>` del lado server (reusa la cadena de `@post`) y un stub `fetch` del lado client que envuelve el handler async en `spawn_local` → set state → re-render. Tipos de punta a punta, cero plumbing (`fetch`/JSON/rutas escritas a mano), cero deps externas. Verificado end-to-end en Chrome real (`examples/view/rpc/`). → [cap 36 de la guía](https://thegreekman76.github.io/fitz/guide/) (frontend `.fitzv`).
...
🎲 **Módulos `rand` / `fs` / `num` + cookies + paridad `run`↔`build` (Hito MatHelp, v0.49.0)** — release combinado del backlog de la primera app real de terceros (`docs/norte-mathelp.md`). **`rand`**: CSPRNG global (`rand.int/float/bool/choice/shuffle/sample/bytes`) + PRNG **sembrado determinístico** (`rand.seeded(N)`, SplitMix64 fijo) que produce la MISMA secuencia en `fitz run` y `fitz build` para siempre (guardás `seed + índice` y reconstruís una partida). **`fs`**: filesystem en runtime (`fs.read/write/append/exists/list/remove/mkdir_all`, `Result<T>` con el path en el error, nunca panic). **`num`**: formateo locale-aware (`num.format/percent/currency` con `es-AR` → `1.234.567,00` / `42,0 %` / `$ 1.250,00` y `en-US`). **Cookies**: leer con `@cookie(name="X")` (decorator apilable como `@header`) + escribir con `Response { cookies: [Cookie { ... }] }` (nominal built-in de 8 campos, cada cookie → un `Set-Cookie`) → login de familia HTML puro, cero JS. **Paridad**: fixes de codegen `-> T?` con `return` (destraba el binario nativo de todo lo que use fitz-liveviews) y `Str + Any`, más un **differ de paridad** que corre un corpus CLI-puro por las dos vías y assertea stdout idéntico bit-a-bit (la red que caza las divergencias `check✓/build✗`). Los 3 módulos con paridad bit-a-bit `fitz run` ↔ `fitz build`. **v0.50.0 (Hito 3/4)**: `Map.remove(key) -> Bool` (muta in place), `is_in(<var>)` en el ORM con una variable `List<T>` → `= ANY($N)` (además del `IN (...)` del literal), paridad `form-urlencoded` → `type` del handler en `fitz run` (login zero-JS idéntico run↔build), y limpieza de codegen (sin paréntesis redundantes en el `match` generado). **v0.51.0 (cierra Hito 3/4 entero — FITZ-02)**: **servido de archivos estáticos** con `@server(static_dir="./public", static_prefix="/static")` — Content-Type por extensión, `ETag` basado en contenido + `If-None-Match` → 304, `Cache-Control`, `Last-Modified`, path-traversal bloqueado; favicon + `manifest.webmanifest` → **PWA instalable** sin nginx al lado. `fitz build --embed-static` hornea los assets en el binario con `include_bytes!` → sirve su propio frontend sin el dir en disco (**distroless**). Paridad bit-a-bit `fitz run` ↔ `fitz build`. → [cap 17 "Cookies y sesiones" + "Archivos estáticos"](https://thegreekman76.github.io/fitz/guide/#17-http-nativo) + caps "Aleatoriedad"/"Filesystem" de la guía.
...
| `fitz dev [--file] [--bin] [--port]` | Hot reload — re-arranca el programa cuando un `.fitz`/`.fitzv`/`fitz.toml` cambia. Con un bin `wasm-client`: buildea + sirve con auto-refresh + preservación de state en el browser (`--port`, default 1234). Editar `fitz.toml` se re-resuelve en vivo. `--bin <nombre>` selecciona el bin en proyectos multi-bin (fullstack: `fitz run --bin server` + `fitz dev --bin web`). |
