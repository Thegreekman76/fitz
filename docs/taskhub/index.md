...
| **[C4](c4-crud-relations-ws.md)** | CRUD + relations + WebSocket en vivo | `@has_many` + `@belongs_to` + companion fields. CRUD de projects + tasks con `.preload("tasks")` para eager loading + scope por ownership. `@authenticated @ws("/ws/projects/{id}")` con `WsConn<TaskEvent>` + broadcast simétrico + limitación honesta del MVP (HTTP handlers no triggerean broadcasts). |
| **[C5](c5-cron-jobs-persistencia.md)** | Cron + background jobs con persistencia | 2 `@cron("...", tz=..., retry=..., store=db_result)` + `@background async fn` + `spawn(...)` desde handler + endpoint admin `GET /api/jobs` lee `fitz_cron_runs`. Tablas auto-creadas. **Sin Celery, sin Redis** — compose sigue con 5 services. |
| **[C6](c6-interop-python-llm.md)** | Interop Python: priorización IA con LLM | `from python import priority` + endpoint `POST /api/tasks/{id}/suggest-priority`. Módulo Python con OpenAI gpt-4o-mini opcional + fallback heurística por keywords. `match Result<Int>` para fallback de emergencia. Cache en `ai_suggested_priority`. **Diferenciador único**: binario nativo Rust + ecosistema Python sin penalty IPC. |
| **[C7](c7-observability-frontend-deploy.md)** | Observability + frontend + deploy production (final) | `@server(prometheus=true)` para `/metrics` + OTel spans → Jaeger. `@healthz`/`@readyz` con check real de DB + SIGTERM drain auto. Frontend vanilla JS (~500 LoC) con login + projects list + board kanban drag&drop + WS live updates. Dockerfile final con `fitz build --bundle-python --bundle-pip openai` → image ~50 MB distroless. |
| **Post-C7** ✅ | **TaskHub publicado como [`boilerplates/taskhub/`](https://github.com/Thegreekman76/fitz/tree/main/boilerplates/taskhub)** | El estado final del C7 publicado al lado de los 9 boilerplates existentes (este es el 10mo y más completo). Clone + `docker compose up -d --build` + bootstrap admin = TaskHub corriendo en ~30s. Sin pasar por los 7 caps. **Cierra el proyecto entero.** |
