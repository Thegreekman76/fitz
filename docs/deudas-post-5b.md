...
**Limitación residual (no bloquea)**: (a) `fitz run` de cron+store persistente sigue con la limitación pre-existente de lifecycle del runtime tokio del intérprete (documentada en v0.11.2) — B20 es fix de **codegen**, no toca el intérprete. (b) Dos módulos importados con el **mismo** nombre de store binding (`db`) causarían shadowing del local en el scope flat del spawn (caso raro; el canónico de un módulo/una conexión funciona). (c) Cron en **main** con `store=X` donde `X` es importado de un módulo no se cubre (escenario distinto, poco frecuente).
...
- **`try_operator_mixed_with_return_status_w13`** y **`auth_codegen_handler_with_body_and_user_w14`** — **ambientales (Windows file-lock)**. Root cause: `Error copying ...\<stem>.exe to ...: El proceso no tiene acceso al archivo ... (os error 32)`. El AV/indexer de Windows retiene el handle del `.exe` recién linkeado más tiempo que la ventana de `copy_binary_with_retry` (8 intentos × 25ms ≈ 200ms). Fallan incluso serial cuando el AV escanea. **No es bug de lenguaje** — se dejan documentados (workaround: correr con AV pausado / stems únicos ya presentes). Candidato futuro: ampliar la ventana de retry de `copy_binary_with_retry` en `src/main.rs` si la flakiness molesta en CI.
