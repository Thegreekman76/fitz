...
### 🟢 K-3 — Component props para compound / nominal / interpolated types en `<Child />` — **CERRADO 2026-07-16** (SSR path)

**Síntoma original**: `<Board initial-cards="{seedCards}" />` no
funcionaba end-to-end. Fitz core Phase 11.5.d shipped `<Child
prop="v" />` composition SOLO con primitivos (`Str`/`Int`/`Float`/
`Bool`/`Nullable<T>` de primitivo). §9.dd extendió STATE
annotations para aceptar cross-file nominals via `from X import Y`.
Pero el path del child PROP coercion sólo aceptaba primitivos, y
`<Card prop="{expr}" />` (interpolación dinámica) abortaba en
expand time con "dynamic prop — deferred to Phase 11.6+".

**Cierre K-3 (post-v0.21.0, 2026-07-16)** — dos sub-releases
coordinados que juntos cierran el gap **enteramente para el SSR
path**:

1. **K-3 List<primitive>** (~130 LoC + 21 tests) — comma-separated
   static props. `<Child tags="a,b,c" />` con `tags: List<Str>`
   coerciona a `vec!["a".to_string(), ...]` en Rust literal (checker
   + WASM emitter) y a `["a", "b", "c"]` en Fitz literal (SSR
   emitter). Empty string → `vec![]` / `[]`. Whitespace around
   commas trimmed. Nested primitives (`List<Nullable<Int>>`, etc.)
   recurse. Both emitter paths share acceptance semantics via
   `check::coerce_child_prop_raw_value` (Rust) + parallel
   `codegen_ssr::coerce_child_prop_raw_value_to_fitz_literal` (Fitz).
   WASM state fields con `List<T>` gain `Vec<Rust>` type +
   `vec![...]` default via `codegen_wasm::type_expr_to_rust` +
   `default_expr_to_rust`.

2. **K-3 Interpolated props (SSR)** (~120 LoC + 5 tests) —
   `<Child prop="{expr}" />` inlina la expresión parseada (via
   `parse_expr_at` en expand time) en el struct literal de la
   composición, aplicando el mismo state-field rewriting rule del
   SSR emitter para templates (`bare_ident` referring to parent
   state field → `state.<field>`). `ChildComponentProp` gains
   `expr: Option<fast::Expr>` field; `is_interpolated()` helper
   discriminates. `format_child_composition` dispatches on it —
   static path uses coerce helper, interpolated path uses
   `format_fitz_expr_scoped` con el `state_field_names` +
   `local_scope` del PARENT (closure-parameter locals de
   `{#for x in xs}` alrededor del `<Child />` shadow correctly).
   El WASM path rechaza interpolación con mensaje claro citando
   Phase 11.7+ (reactive propagation from parent state to
   mounted child needs child-lifecycle hooks). Expresiones ricas
   funcionan naturalmente vía la scoping helper (`{n + 1}`,
   `{title}`, method calls, etc.). Nominal types funcionan via
   interpolation — `<Card user="{seed}" />` con `seed: User` en
   parent state emite `user: state.seed` en el struct literal.

**Con esto Board.fitzv migration queda destrabada para el SSR
target** (fitz-liveviews). El pattern canónico:
```fitzv
component App {
  state { cards: List<Card> = [] }
  <template>
    <Board initial-cards="{cards}" />
  </template>
}
```
funciona bit-a-bit en el SSR emitter.

**Deudas residuales derivadas** (NO bloquean Board):

- **Client-WASM path para interpolated props** — el WASM emitter
  hoy rechaza con mensaje claro pointing at Phase 11.7+. La
  razón real es plumbing de reactivity: cuando el parent muta
  `state.cards`, el `<Board />` mounted no se re-renderiza sin
  child-lifecycle hooks + reactive-prop watchers. Refinable
  cuando aparezca demanda cliente-side.
- **`Map<K, V>` static props** — sintaxis `<Child meta="k=v,x=y" />`
  o similar; requiere decisión sobre key/value separator syntax.
  Workaround: interpolación (`meta="{someMap}"`) cubre el caso.
- **Nominal-type STATIC props** — hoy solo interpolados. Si el
  user quiere un literal inline (`<Card user='User { id: 1 }' />`)
  necesita interpolación o extraer a `let x = User { ... }` +
  `<Card user="{x}" />`. El pattern con interpolación es más
  natural y cubre el 100% del caso.
- **Checker type-check del expr interpolado vs field type** —
  actualmente skippeado (trust runtime). Si false negatives
...
- **K-1** shipped en fitz-liveviews v0.5.0 con `dispatch_to()`
  explicit event API.
- **K-2** shipped en fitz-liveviews v0.5.0 con `component_state()`
  + `set_component_state()`.
- **K-3** shipped en Fitz core post-v0.21.0 con dos slices:
  List<primitive> static + interpolated props (SSR).

**Full Board.fitzv migration está desbloqueada** para SSR. Kanban
SPA port + client-side dynamic capabilities siguen para Phase 11.7+
...
**Limitaciones conocidas** (documentadas): (a) el shared state de módulo debe ser referenciado por HANDLERS del MISMO módulo (no cross-module — `db` en jobs.fitz no lo comparten los handlers de posts.fitz; cada módulo declara su propio `let db`, que POOL_CACHE dedupea a la misma conexión); (b) ~~sync shared-state primitivo (Int/Bool) mutable compartido cae a const/accessor y no a `__FITZ_STATE_X`~~ — **CERRADO en v0.37.14** (ver abajo): la nota original ("caso irreal") era incorrecta; el showcase Admin ABM lo pegó de verdad con `let PAGE_SIZE: Int = 8` de módulo usado por handlers `@ws`, y el const short-circuit rompía `fitz build` (E0425 `__FITZ_STATE_PAGE_SIZE` faltante + E0530 `let` shadoweando `const`).

### 🟢 v0.37.14 — el sub-caso (b): primitivo shared-state de módulo (2026-08-13)

`gen_module_top_let` computa `is_shared_state = state_var_types.contains_key(name)` al tope y gatea Paths 1a (Str literal) y 1b (const primitivo Int/Float/Bool) en `!is_shared_state`. Un primitivo shared-state cae ahora a una rama nueva que emite `static __FITZ_STATE_X: std::sync::LazyLock<T>` + accessor `pub fn X()` (mirror bit-a-bit del caso main en `gen_http_main`, que ya emitía el `LazyLock` para TODO state var). Los primitivos shared-state se registran en `accessor_consts` tras `resolve_state_var_types` para que una fn NO-handler del módulo que los referencie bare emita `X()` (las fns handler siguen shadoweando con el local materializado por `gen_top_fn`). Paridad bit-a-bit `fitz run` ↔ `fitz build` validada con curl (repro mínima de 2 archivos). Test E2E `compile_e2e::module_shared_state_primitive_const_compiles_v0_37_14`. **Desbloquea el `fitz build` del showcase Admin ABM** de fitz-liveviews. Cambio contenido en `src/codegen.rs` (~50 LoC), sin impacto en LSP/grammar (extensión VSCode sin cambios más allá del bump de versión).
...
**(1) Accessors tipados de `DbRow` en el intérprete.** Una fila cruda de `conn.query(sql, args).await?` la tipa el checker como `DbRow` (`get_int`/`get_str`/`get_float`/`get_bool`/`len`, cada uno `Result<T>`) y el codegen emite `__FitzDbRow` con esos métodos; las docs (`db-orm.md`, guide cap 31) los documentan. Pero el **intérprete** devolvía las filas como `Value::Map` (via `pg_row_to_fitz_map`), así que `r.get_str("col")` en `fitz run` moría en runtime con *"`Map` has no method named `get_str`"* aunque `fitz check` y `fitz build` lo aceptaran. Fix (`src/evaluator.rs`): nueva `db_row_get(receiver, args, span, method)` + 4 arms en el dispatch de métodos de `Value::Map` (`get_str`/`get_int`/`get_float`/`get_bool`; `len` ya existía). Semántica + mensajes de error espejo del codegen: col ausente → Err "no existe en el row"; NULL → Err "es NULL"; tipo PG que no matchea → Err "is X, expected Y". El checker gatea el uso sobre `Map` genérico (solo `DbRow` expone estos métodos), así que agregar los arms al dispatch de `Map` es un superset seguro. Unit test `evaluator::tests::db_row_get_typed_accessors_parity_v0_37_3` (happy path por tipo + wrong-type/NULL/missing → Err). Cierra la divergencia; `r.get_str()`/`.get_int()` son parity-safe.

**(2) `has_role_field` cross-module cuando el `User` está en un módulo hermano.** `@admin`/`@requires` exigen que el `User` (return del `@auth_provider`) tenga un campo `role: Str`. La detección cross-module (`extract_auth_provider_signature`, W12) computaba `has_role_field` mirando **solo los `TypeDef` del módulo del provider**. En el layout multi-archivo canónico el módulo del provider **importa** su `User` (`auth.fitz` hace `from models import User`, `User` vive en `models.fitz`), así que `role` era invisible → `has_role_field = false` → `@admin` fallaba en `fitz build` con *"type `User` must have a `role: Str` field"* — aunque `fitz check` (que usa el `TypeEnv` enriquecido) pasara. Fix: helper `pub fn type_decl_has_role_field(program, type_name)` en `src/types.rs` + en los pre-scans cross-module (`pre_scan_imported_auth_provider_for_loader` del codegen y `pre_scan_imported_auth_provider_lsp` del LSP), cuando `!has_role_field`, seguir los imports del módulo del provider para encontrar `type User { role: Str }` en el módulo hermano. `main.rs`/`fitz check` no necesitan el fix (ya resolvían vía el TypeEnv full-program). Test E2E `compile_e2e::admin_cross_module_role_field_via_provider_module_imports_v0_37_3` (3 módulos: `models.fitz` con `User`, `auth.fitz` con el provider que lo importa, main con `@admin`). El LSP se arregla en paralelo para no mostrar un squiggle falso.

**Validado end-to-end** sobre `api-orm-full` (multi-archivo): `@cron("...", store=cron_db)` en `jobs.fitz` persiste runs + `@admin GET /jobs` los lee con `r.get_str()`/`.get_int()` — smoke real contra Postgres local, `fitz build` verde con `@admin`, `GET /jobs` sin token → 401. Paridad `fitz run` ↔ `fitz build`.

**Deuda residual derivada** (NO bloquea): `main.rs::pre_scan_imported_auth_provider` (path del checker full-program) no recibió el mismo fix de cross-import porque ese path ya resuelve `role` vía el `TypeEnv`; si aparece un caso donde el checker full-program también falle, aplicar el mismo helper ahí.

---

## 🟢 B20 — `@cron(store=X)` cross-module CERRADO (2026-08-07)

**Detectada al destrabar fitzwatch** después de cerrar B19. Caso edge no soportado por el codegen: `@cron(..., store=X)` declarado en un módulo importado (e.g., `scheduler.fitz`) referencia un binding `X` que resuelve al `let X = db.connect(...).await` también declarado en ese módulo. Pre-fix el spawn cross-module emitía `(&X).into_store()` en `fn main()` del crate root, donde `X` no estaba en scope → `error[E0425] cannot find value 'X' in this scope`; y el módulo emitía un `pub fn X() { ...await }` roto (cuerpo async en fn sync).

**Fix (2026-08-07, solo `src/codegen.rs` + test)** — hoist a `OnceCell` crate-visible, drive del init + materialización en el spawn:
- **Detección**: helper libre `collect_cron_store_var_names(program)` walka los `@cron` de un módulo y extrae los `store=<ident>`. En el loop de carga de módulos se computa además `module_hoisted_cron_store_vars` = store vars que ADEMÁS están definidos como `let async` en el mismo módulo (predicado idéntico a `gen_module_top_let`). Se guarda en `LoadedModule.hoisted_cron_store_vars`.
- **Emisión en el módulo** (`gen_module_top_let`, gated en `module_cron_store_vars.contains(name) && expr_contains_await(value)`): en vez del accessor roto `pub fn X()`, emite `pub(crate) static __FITZ_STATE_X: tokio::sync::OnceCell<T>` + `pub(crate) async fn __fitz_init_state_X() { let __init: T = <rhs>; let _ = __FITZ_STATE_X.set(__init); }`. El `let _ = ...set(...)` evita el bound `Debug` de `.expect()` y es idempotente.
- **Tolerancia del pre-pass** (`collect_module_sigs`, threaded con el mismo set): un `let X = db.connect(...).await` sin anotación se acepta para los cron-store vars (antes rechazaba todo RHS no-literal de módulo con "RHS is not a literal — annotate the type"). El `const_sig` es irrelevante (el binding solo alimenta `store=X`).
- **Materialización en `emit_cron_job_spawns`**: itera `self.xmod_cron_store_hoists` (poblado en la construcción de `cron_jobs_info` desde `loader.modules[*].hoisted_cron_store_vars`) y emite `crate::<mod>::__fitz_init_state_X().await;` + `let X = crate::<mod>::__FITZ_STATE_X.get().expect(...).clone();` antes de los spawns. `(&X).into_store()` ya resuelve el local materializado.

**Distinción clave que preserva b19_derived**: solo se hoistea el binding **co-localizado** en el módulo. Un cron en un módulo con `store=db` donde `db` vive en **main** (el caso b19_derived) deja `hoisted_cron_store_vars` vacío → ese `db` sigue emitido como local normal de main y alcanzado como `db` pelado (mecanismo 2). Sin esta distinción, el fix habría roto b19_derived.

**Tests verdes**: `cron_in_imported_module_is_spawned_b19` + `cron_with_persistent_store_in_imported_module_b19_derived` (sin regresión) + nuevo `cron_store_binding_in_imported_module_compila_b20` (`let db` + `@cron(store=db)` ambos en el módulo importado, compila a binario). **Smoke real Postgres local**: el binario crea `fitz_cron_jobs`/`fitz_cron_runs`, arranca el scheduler y persiste 3 runs `status=ok`. Cierra el workaround canónico (declarar en main) que TaskHub/fitzwatch usaban.

**Limitación residual**: (a) **CERRADA v0.37.3** — `fitz run` de cron+store persistente ahora funciona con paridad ante `fitz build`, en cron-only Y HTTP+cron. **Root cause** (mapeado 2026-08-07): el intérprete construía el runtime #1 (`current_thread`) para el eval, que abría el `TcpStream` de Postgres + cacheaba el `Arc<DbConnHandle>` en un `POOL_CACHE` global; el eval retornaba y **runtime #1 se dropeaba**; `serve`/`run_scheduler_only` construía runtime #2 (`multi_thread`) y la primera I/O del cron (`init_storage`) manejaba el `TcpStream` atado al reactor #1 ya cerrado → *"A Tokio 1.x context was found, but it is being shutdown"*. **Fix** (~55 LoC, solo `run_file` + `serve` + `run_scheduler_only`): `run_file` construye UN runtime `multi_thread` compartido up-front (`http::build_server_runtime`, única fuente de la config 16 MB + `enable_all`), corre el eval con `shared_runtime.block_on(eval_with_base_and_deps(...))` (mismo `block_on` en el main thread que antes → mismo stack, cero cambio de comportamiento del eval) y drivea serve/scheduler sobre ESE mismo runtime vía `serve_on_runtime`/`run_scheduler_on_runtime` (las viejas `serve`/`run_scheduler_only` quedan como wrappers que construyen su propio runtime, backward-compat). Como el runtime compartido vive todo el proceso, el `TcpStream` abierto durante el eval sigue atado a un reactor vivo cuando el scheduler corre `CREATE TABLE fitz_cron_jobs`. Confirmado por Explore: `Value`/`EnvRef` son `Arc<parking_lot::Mutex>` (F17.2) y `serve` ya invocaba `Value::Function` del eval en un runtime `multi_thread` → eval-on-multi_thread ya estaba probado; el único punto runtime-bound era la health-check task (`tokio::spawn` en `db.rs`) + el `TcpStream`. Test E2E nuevo `tests/cron_run_real_postgres.rs` (`#[ignore]` opt-in; spawnea `fitz run` real; cron-only + HTTP+cron; verifica >= 1 run `ok` persistido) verde contra Postgres local; los 5 cron E2E de codegen + smoke guía (290) sin regresión; smokes manuales de `fitz run` (CLI/async/HTTP/DB-HTTP) sin regresión. **(b) CERRADA v0.37.2**: dos módulos importados con el mismo nombre de store binding (`db`) causaban shadowing del local en el scope flat del spawn → alpha's cron corría contra beta's conexión (bug silencioso). Fix: locales module-qualified únicos (`__fitz_cron_store_<mod>_<var>`) + `store_field` resuelto por `(module_path, store_var)`. Test `cron_store_two_modules_same_name_use_distinct_locals_b20_residual_a`. **(c) safe-by-failure**: cron en **main** con `store=X` donde `X` es importado de un módulo falla loud en compile-time (loader "RHS is not a literal" para el caso natural, rustc para el annotated) — nunca conexión silenciosa equivocada. Soporte completo (materializar un store importado desde main) requiere refactor de ordering del loader; diferido (topología rara). Test guard `cron_store_imported_binding_in_main_fails_loud_b20_residual_b`.

---

## 🟢 Triage compile_e2e — 2 fixes + 2 ambientales caracterizados (2026-08-07)

Barrida de los failures de `compile_e2e` (los "8 pre-existentes" que arrastraba la nota de cierre v0.16.0). Corrida completa: **392 passed, 5 failed**; re-corrida `--test-threads=1` deja los failures reales aislados. Diagnóstico y acción por test:

- **`auth_codegen_cross_module_provider_w12`** — pasó al serializar → era **flake de paralelismo** (varios `fitz build` compitiendo por `target/fitz-build/`). No es bug.
- **`try_operator_mixed_with_return_status_w13`** y **`auth_codegen_handler_with_body_and_user_w14`** — **ambientales (Windows file-lock)**. Root cause: `Error copying ...\<stem>.exe to ...: El proceso no tiene acceso al archivo ... (os error 32)`. El AV/indexer de Windows retiene el handle del `.exe` recién linkeado más tiempo que la ventana original de `copy_binary_with_retry` (8 intentos × 25ms lineal ≈ 700ms). Fallaban incluso serial cuando el AV escanea. **No es bug de lenguaje**. **Mitigado v0.37.2**: la ventana de retry se amplió a **20 intentos con backoff exponencial capeado (50→400ms, ~6.7s worst case)** en `src/main.rs::copy_binary_with_retry`; los sleeps solo ocurren en un retry por `os error 32`, el happy path es instantáneo. Debería estabilizar estos flakes en máquinas con AV agresivo.
- **`orm_w17_eager_loaded_virtuales_aparecen_en_json`** — **test STALE, CERRADO**. El `fitz build` tiene éxito; el assert de inspección estática buscaba el literal `if !__g.is_empty() { __obj.insert("posts"` que driftó cuando el codegen se refactorizó para acotar el `MutexGuard` (ahora `let __is_empty = { let __g = ...; __g.is_empty() }; if !__is_empty { ... }`). El conditional has_many se emite y es funcionalmente idéntico (skip del field si la relación está vacía). Fix: actualizar el assert al string actual. El assert del companion `author` (`if self.author.is_some()`) sí matcheaba.
- **`loader_absoluto_data_sibling_import_compila_en_fitz_build`** — **bug real del loader de codegen, CERRADO**. Root cause: el fallback `import_root` existe en el evaluator (`fitz run`, desde la mini-fase loader-absoluto 2026-05-22) pero **nunca se portó al loader de `fitz build`**. `from types.user import User` desde `src/data/users.fitz` buscaba solo relativo a `base_dir=src/data/` → `src/data/types/user.fitz` (no existe) → `module types.user not found`, sin caer al sibling `src/types/user.fitz`. Fix: portar el patrón probado del evaluator — `ModuleLoader` suma campo `import_root: PathBuf` (fijo al dir del entry), `resolve_path` → `resolve_path_candidates` devuelve candidatos (relativo a `base_dir` PRIMERO, luego a `import_root` si difiere; `.fitz` antes de `.fitzv` en cada dir), y `load_module` canonicaliza el primero que exista (cita el primer candidato en el error not-found, preservando el mensaje histórico). Deps resuelven bajo su propio root sin fallback. Paridad con `fitz run`.

**Verificación**: fmt + clippy `--lib --tests -D warnings` (default + lsp) limpios; unit 4026 (default) / 4187 (lsp); subset amplio de `compile_e2e` (`cross_module` + `loader` + `orm` + `cron`) + smoke de la guía (290 ejemplos) corrido `--test-threads=1` → **56 passed, 0 failed** (cubre el blast radius del resolver cross-module que toca el fix del loader, sin flakes de file-lock al serializar).
...
**Causa raíz (layer 2, la que quedaba)**: el `Member { ... }` que llegaba a `gen_struct_lit` en el ctx de **main** NO venía del struct lit `TeamPanel {}` del `flv_register` auto-inyectado (ese path ya delegaba, PreF8.3), sino del **`impl __FromFitzJson for <T>Data` cross-module** que `emit_helpers_for_imported_types` emite en main.rs cuando hay HTTP: el arm `None =>` (field ausente en el JSON con default declarado) inlineaba el default expr vía `self.gen_expr(default_expr)` en el ctx de main (`src/codegen.rs::gen_type_http_impls_for_sig_with_meta_and_mode`). Para `TeamPanel.members` el default es `[Member { ... }]` y `Member` es **transitivo** (importado por el módulo del componente, no por main) → no está en `type_sigs` de main → error. El caso plano del mismo módulo funcionaba porque no pasa por ese emit (y `gen_struct_lit` de tipos importados ya delegaba al helper del módulo).

**Fix (hipótesis b de la caracterización, quirúrgica)**: cuando `gen_type_http_impls_for_sig_with_meta_and_mode` corre con contexto cross-module (`xmod = Some((module_index, orig_sig))`, el mismo plumbing de W19), los dos sitios que materializan defaults — el arm `None =>` del match del field y el init de fields `@hidden` en el struct literal — emiten `<mod_qualifier>::__default_<T>_<field>()` en lugar de inlinear el `gen_expr(default_expr)`. Esos helpers los emite `gen_type_default_helpers` (PreF8.3) en cada módulo para todo `type` con defaults, con el scope correcto del módulo definidor, y devuelven el tipo Rust exacto del field (consistente con el render W19 `rust_type_for_imported_body`). El sentinel `Str = ""` para Date/DateTime/Uuid mantiene su rama previa (no gen_expr, no cambia). Tipos locales de main (`xmod = None`) siguen inlineando igual que antes.

**Test** (+1 E2E): `compile_e2e::cross_module_default_with_transitive_nominal_delegates_to_module_helper_w26` — 3 archivos (`models.fitz` con `Member`, `panel.fitz` con `type Panel { title: Str, members: List<Member> = [Member{...}] }`, main HTTP que importa solo `Panel`); asegura build OK + main.rs delega a `panel::__default_Panel_members()` sin inlinear el literal + panel.rs emite el helper.

**Validado end-to-end** sobre `examples/course/c3-team-panel-sfc` (fitz-liveviews): `fitz build` verde; binario sirve `GET /` con las 3 members y estados correctos; toggle por WS (frame sin `html`/`patches` — ejercita en runtime el nuevo path `fitz_liveviews::__default_LiveFrame_patches()`) muta el estado con paridad exacta ante `fitz run` (HTML idéntico módulo line endings — artefacto preexistente de literales multi-línea, no de W26).

**Deuda residual derivada** (NO bloquea): el path Python espejo (`py_field_extract_code`, paso 2 "inline gen_expr" del default) tiene el mismo patrón latente — solo dispara con `uses_python` + tipos importados con defaults de nominales transitivos + coerción PyDict→Instance; se cierra con la misma delegación si aparece el caso real. Bonus caracterizado de paso: `return Panel {}` (struct lit VACÍO en return position) parsea como Ident + bloque y muere con "unknown variable in codegen: `Panel`" — con al menos un field provisto funciona; gap de parser menor, separado de W26.
...
**Gap 1 — `coerce(Map<K,Function> → Map<K,Any>)` faltante** (`src/codegen.rs::coerce`): el `flv_register("Name", state, render_fn, {"event": handler, ...})` auto-inyectado (v0.20.1) pasa un `Map<Str, Any>` de funciones. El LUB del map literal lo tipaba `Map<Str, Function>` (homogéneo), y al coaccionarlo al parámetro `Map<Str, Any>` no había rama `(Map, Map)` → los `Arc<dyn Fn>` crudos sobrevivían → E0308 (`expected __FitzValue, found Arc<...>`). **Fix**: ramas nuevas `(Map<K,Function> → Map<K,Any>)` y `(List<Function> → List<Any>)` que reconstruyen el contenedor envolviendo cada función en `__FitzValue::Function(...)` vía `wrap_fn_as_any` (el adapter que marshalea `Vec<__FitzValue>` ↔ tipos concretos por la firma; ya existía y `coerce(fn→Any)` ya lo usaba — solo faltaba la rama del contenedor). La key también se coacciona a `__FitzValue` porque el rep Rust de `Map<K, Any>` es `Vec<(__FitzValue, __FitzValue)>`. **Guard estrecho**: solo cuando el value es `Function` concreta ensanchando a `Any` — un rewrap general `Map→Map` sobre-disparaba en las coerciones internas del propio módulo lib (`Map<Any,Any>→Map<Str,Str>` etc.) y emitía `__fv_to_*` en archivos de módulo que no los importan. El caso función-widening solo ocurre en main.rs (el flv_register auto-inyectado), donde los helpers viven.

**Gap 2 — globals de módulo mutables perdían estado** (`src/codegen.rs`, path del `let` top-level de módulo, "Path 3"): un `let X = {}` a nivel de módulo (Map/List/Nominal mutable) se emitía como `pub fn X() -> T { <default fresco> }` — devolvía un valor **nuevo vacío en cada llamada**. El estado escrito a un global de módulo se perdía silenciosamente entre llamadas y entre threads worker de HTTP. Esto rompía el `COMPONENT_REGISTRY`/`COMPONENT_STATE_STORE` de fitz-liveviews (`flv_register` poblaba una instancia, `component()` leía otra vacía → "key not found in map"). **Fix**: globals de módulo de tipo referencia (List/Map/Nominal, rep Rust `Arc<Mutex<...>>`) se emiten como `static __FITZ_GLOBAL_<NAME>: LazyLock<Arc<Mutex<T>>>` + getter que clona el Arc (misma instancia compartida). Espejo del state HTTP del main (F17.4b). Alinea `fitz build` con `fitz run` (un global de módulo es UNA instancia). Primitivos siguen por const/static.

**Tests** (+3): `compile_e2e::coerce_map_of_functions_to_map_any_wraps_in_fitzvalue_function_w25` (repro aislado: map de 2 fns → `Map<Str,Any>`, build+run → `2`), `compile_e2e::module_mutable_global_persists_state_via_shared_lazylock_w25` (repro aislado: write 42 a global de módulo, read back → `42` en binario), `codegen::tests::module_let_map_top_level_emits_shared_lazylock_global_w25`.

**Validado**: counter SFC (C1) y dashboard de tiles (C5, multi-instancia) compilan a binario + paridad run↔binario exacta (aislamiento per-instancia downloads=2/signups=1/errors=0 en el binario).

**Deuda residual derivada (v0.28.4+, para que el admin ABM use LiveComponents completas)**: SFCs con estado `List` pegan `.raw` sobre `Any` en el render de listas del emisor SSR; SFCs con nominales importados en el estado pegan `unknown type X in codegen` (primo de W17/W19 cross-module). El core (registro + dispatch + estado per-instancia) ya compila; falta el long tail de features del path SSR→binario.
...
## 🟢 Consistencia checker↔codegen — clase B16 (CERRADA v0.40.1)
...
(v0.39.1), `if`-como-expresión como LUB de ramas (v0.39.3), y **los dos últimos
residuales en v0.40.1** (R-if-div + R-list-mix, abajo). **No quedan ítems
abiertos conocidos de esta clase.**

**Residuales ~~abiertos~~ CERRADOS v0.40.1** (requerían cambio de codegen además
del checker):

### 🟢 R-if-div — `if c { A } else { return B }` usado como valor — **CERRADO v0.40.1**

**Síntoma (histórico)**: `let n = if (o != null) { o } else { return 0 }` seguido
de `n + 1` → `fitz check` pasaba (checker tipaba el `if` como `Any`) pero `fitz
build` fallaba con `operador + no aplicable a Null y Int en codegen`. El codegen
(`gen_if_expr`) trataba el `if` con rama divergente como statement-mode → `Null`,
aunque Rust tipa el `if` como `A` (la rama `else` es `!`).

**Cierre (v0.40.1)**: dos cambios coordinados. (1) Codegen `gen_if_expr` gana una
rama `asym_value` (helper nuevo `block_diverges`): cuando una rama tiene value
tail y la otra diverge (`return`/`break`), emite expression-mode con el tipo de la
rama-valor (la divergente es `!` y coerce), en vez del statement-mode `Null`. La
rama `want_value` simétrica queda byte-idéntica (arm separado). (2) Checker
`Expr::If` replica: want_value → LUB; asimétrico (una rama Expr, otra diverge) →
el tipo de la rama-valor; resto → `Any`. Ahora `let n: Int = if c { A } else {
return B }` chequea + buildea + corre. Validado con E2E
`compile_e2e::v040_1_divergent_else_if_builds_and_runs` (unwrap_or(41)=42,
unwrap_or(null)=0) + 2 unit tests.

### 🟢 R-list-mix — literales `[...]`/`{...}` heterogéneos vs anotación — **CERRADO v0.40.1**

**Síntoma (histórico)**: `let xs: List<Int> = [1, 2.5]` → `fitz check` pasaba (el
checker tipaba `List<Any>` con "primer elemento o Any") pero `fitz build` fallaba
con `E0308` (el codegen computa `lub(Int, Float) = Float` → `Vec<f64>`).

**Cierre (v0.40.1)**: `Expr::List`/`Expr::Map` en el checker computan el LUB de los
elementos con un **sticky Any** (helper nuevo `sticky_lub`), replicando bit-a-bit
`codegen::gen_list_lit`: compatibles-pero-distintos → LUB (`[1, 2.5]` →
`List<Float>`); genuinamente incompatibles → `List<Any>` sticky (`[1, "dos", 2]` →
`List<Any>`, el codegen emite `Vec<__FitzValue>`, el intérprete lo permite). Ahora
`let xs: List<Int> = [1, 2.5]` errora en `fitz check` con mensaje claro. 3 unit
tests (`list_literal_*`, `map_literal_*`) + E2E
`compile_e2e::v040_1_list_lub_float_builds_and_runs`.
...
2. **L2 — Helper `with_temp_output`** en codegen (Baja) — ✅
   **CERRADO** (2026-08-07, deuda stale): el helper genérico
   `fn with_temp_output<R>(&mut self, f: impl FnOnce(&mut Self) -> R)
   -> (String, R)` (`codegen.rs:29244`) ya existe con 11 call sites;
   el único `mem::take(&mut self.output)` restante es su propia
   implementación. El refactor se hizo en algún sub-paso posterior a
   la anotación de esta deuda; queda como no-op documental.
