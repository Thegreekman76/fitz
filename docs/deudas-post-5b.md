...
**Limitación residual**: (a) `fitz run` de cron+store persistente sigue con la limitación pre-existente de lifecycle del runtime tokio del intérprete (documentada en v0.11.2) — B20 es fix de **codegen**, no toca el intérprete. **Diagnóstico completo mapeado (2026-08-07)**: el intérprete construye el runtime #1 (`current_thread`) para el eval, que abre el `TcpStream` de Postgres + cachea el `Arc<DbConnHandle>` en un `POOL_CACHE` global; el eval retorna y **runtime #1 se dropea**; `serve`/`run_scheduler_only` construye runtime #2 (`multi_thread`) y la primera I/O del cron (`init_storage`) maneja el `TcpStream` atado al reactor #1 cerrado. Fix = refactor **moderado** (~60-120 LoC: unificar en UN runtime en `main.rs`+`http.rs`+`cron_jobs.rs`) — diferido a sub-paso dedicado por el riesgo de regresión en todo `fitz run`. **(b) CERRADA v0.37.2**: dos módulos importados con el mismo nombre de store binding (`db`) causaban shadowing del local en el scope flat del spawn → alpha's cron corría contra beta's conexión (bug silencioso). Fix: locales module-qualified únicos (`__fitz_cron_store_<mod>_<var>`) + `store_field` resuelto por `(module_path, store_var)`. Test `cron_store_two_modules_same_name_use_distinct_locals_b20_residual_a`. **(c) safe-by-failure**: cron en **main** con `store=X` donde `X` es importado de un módulo falla loud en compile-time (loader "RHS is not a literal" para el caso natural, rustc para el annotated) — nunca conexión silenciosa equivocada. Soporte completo (materializar un store importado desde main) requiere refactor de ordering del loader; diferido (topología rara). Test guard `cron_store_imported_binding_in_main_fails_loud_b20_residual_b`.
