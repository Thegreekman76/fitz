...
**Síntoma** (uno solo, paralelo al Síntoma 2 de v0.19.5 pero del otro lado de la importación): proyecto multi-archivo donde `mw.fitz` declara la fn middleware (async, con `req: Request`), `handlers.fitz` aplica `@middleware(mw_strict) @post(...)` a un handler local (módulo importer), y `main.fitz` solo importa el handler para mountarlo. `fitz check` pasa limpio (es bug del codegen, no del checker), `fitz build` aborta con E0425/E0422 sobre el `handlers.rs` emitido.

**Fix** (~50 LoC netas, 2 archivos `src/codegen.rs` + `tests/compile_e2e.rs`, 1 E2E test). Heurística (b) "trivial" de las 3 opciones evaluadas pre-fix (a estricta, b trivial, c estructural): detector nuevo `program_has_handler_with_middleware(program: &Program) -> bool` walka el AST del módulo buscando `Stmt::FnDef` con decorator `@get`/`@post`/`@put`/`@delete` + al menos un decorator `@middleware(...)`, sin importar si el ident del middleware resuelve local o cross-module. El call site existente (`generate_module_rs_with_bindings` ~línea 5320) que decide emitir `use crate::{Request, RequestData}` agrega el nuevo predicado como tercer OR a los dos existentes (`module_uses_request_local` + `module_has_imported_middleware_fn`). Costo del `use` extra en módulos benignos despreciable (rustc dead-code elimina si no se usa) y elimina la posibilidad de regresión del bug en cualquier sentido del cross-module.

**1 E2E test nuevo** (`tests/compile_e2e.rs::v019_6_cross_module_middleware_applied_in_importer_module_emits_request_imports`): shape canónica con 3 archivos (mw.fitz declara `async fn mw_strict(req: Request) { return null }`, handlers.fitz hace `from mw import mw_strict` + aplica `@middleware(mw_strict) @post("/protected") fn protected() -> Str => "ok"`, main.fitz hace `from handlers import protected` + `@server(43960) fn main() => 0`). Inspecciona el `handlers.rs` emitido confirmando `use crate::{Request, RequestData}` (o split forms). Paralelo bit-a-bit a `v019_5_cross_module_middleware_fn_con_request_arg_compila` que cubre el otro lado (módulo del middleware).

**Workaround user-land aplicado pre-v0.19.6 (removible al bumpear)**: fn dummy `_codegen_request_anchor(req: Request) -> Bool => true` declarada al inicio de los módulos importer del middleware forzaba al detector `program_uses_request_type` a disparar (el AST del módulo tenía `Request` en TypeExpr del param de la fn dummy). Costo: 1 fn extra por módulo (zero cost al runtime). Tras bumpear el binario el workaround se quita y el patrón canónico cross-module funciona limpio.

**Tests al cierre v0.19.6**: 3201 lib (sin cambios — el fix se prueba a través del E2E nuevo) + 98 cli_e2e + 385 compile_e2e (+1 nuevo `v019_6_*`) + 3 openapi_e2e. fmt + clippy (default + lsp) `-D warnings` limpios. **Verificación pre-bump completa**: lib suite 3201/3201 verde, cli_e2e 98/98 verde, openapi_e2e 3/3 verde, los 14 tests del bloque `v019_*` (incluido el nuevo) 14/14 verde, smoke `GUIDE_EXAMPLES_COMPILE` (~370 ejemplos guía+curso+TaskHub) verde en 748s ≈ 12.5 min, 11/11 boilerplates `fitz check` verde. Bump Cargo.toml `0.19.5` → `0.19.6` + extensión VSCode `0.19.5` → `0.19.6` + `.vsix` regenerado bundleando el `fitz-lsp.exe` fresh.

**Impacto en producción**: cualquier proyecto con el patrón canónico cross-module middleware multi-archivo (módulo dedicado a rate limiting / audit / observability + handlers en módulos separados) puede ahora compilar a binario nativo sin workaround. Cierra completamente el patrón que v0.19.5 prometió destrabar — el sub-caso del módulo importer era el otro lado de la moneda.

**Deudas residuales derivadas de v0.19.6** (NO bloquean uso real, no abre nada nuevo): ninguna. Las deudas residuales heredadas de v0.19.5 (async middleware fns en `fitz run` intérprete, LSP cross-module pre-scan `@middleware`, wrap-style middleware con `Fn() -> Response` second param) siguen vivas pero no son específicas de v0.19.6.

**Próximo norte tras v0.19.6**: smoke real en cualquier proyecto con el patrón cross-module multi-archivo (rebuild binario fresh + bump versión + remover workarounds `_codegen_request_anchor`). Ningún ítem crítico abierto del stack web (server + client + WS + auth + SMTP + Response + observability + jobs + ORM + middleware cross-module completo todos cerrados).
...
- **T1 — bloquean M5** (cerrar antes de arrancar a escribir
  ...
  - **9.w.3.iter2 — Persistencia + retry + timezone + catch_up
    ...
    - 9.w.3.iter2.e: cap 30 de `docs/guide.md` "Jobs sin
      ...
      conocida documentada: `fitz run` con `store=db` tenía un bug
      del flujo de runtimes tokio del intérprete (afecta cron-only
      Y HTTP+cron; workaround: `fitz build` — sumar un handler
      HTTP NO lo evita). **[CERRADO en v0.37.3: `run_file` unifica
      el eval + serve/scheduler en un único runtime compartido; el
      bug de lifecycle ya no existe y `fitz run` tiene paridad con
      `fitz build`.]** Sub-sección
...
- 🟡 **Fase 11 (Frontend en `.fitz`)** — **arrancada 2026-07-14,
  Phase 11.1 → 11.5 + 11.6.a/b/c/d + 11.6.e §9.z/aa/bb shipped
  in v0.21.0 (2026-07-16)**. Full detail exhaustivo en
  [`docs/fase-11-plan.md`](fase-11-plan.md) con §9.a–§9.bb
  cubriendo cada sub-paso. **Sub-fases cerradas al 2026-07-16**:
  11.1 (POC parser + `.fitzv` extension), 11.2.a/b/c (bridge
  classic AST + checker + directives + `<slot>`), view-lexer §7
  (state annotations con generics), 11.3.a/b/c (scoped styles +
  CSS mini-parser + `apply_scope`), 11.4.a/b/c/d (WASM emitter
  approach A2 hand-rolled `wasm-bindgen` bajo feature opt-in
  `client-wasm`; bundle-size gate cerrado 2026-07-15 con 11.4 KB
  gzipped sobre 40 KB, 28.6 KB headroom; browser smoke manual
  Windows 11 / Chrome validado), 11.5.a/b/c/d/e (CLI wiring +
  manifest `[[bin]]` array + wasm-client emit + multi-component
  composition `<Child prop="v" />` + cierre formal — **cierra
  Phase 11.5 entera**; **también cierra debt 9.y.8+ multi-bin**),
  11.6.a/b/c/d (SSR emitter research + skeleton + full expression
  grammar + `{#if}` / `{#for}` template directives + view-lexer
  `.` fix + module loader integration para `.fitzv` transparente +
  same-file `<Child prop="v" />` composición), **11.6.e PARTIAL**
  (§9.z SSR emitter `payload` scope + enriched `fitz_liveviews`
  missing-dep hint; §9.aa event-body widening con `if`/`let`/
  nested guards + `Expr::If`/`Expr::StructLit` on RHS; §9.bb
  cross-module `@live_component` auto-inject paralelo a W12/B10
  removes manual `flv_register(...)` boilerplate para components
  declarados en imported `.fitzv`/`.fitz` sibling modules). **Sub-
  fases abiertas**: 11.6.e continuation (cross-file `<Child />`
  composition — §9.y debt, low priority porque ninguno de los 4
  ejemplos lo necesita; `fitz check` inject-time errors —
  refinable si LSP or CI-only flows demand it; migration commits
  en `fitz-liveviews` — counter draft uncommitted, dashboard/
  chat/kanban land post-v0.21.0), **11.7** (client-side dynamic
  capabilities + kanban SPA port), **11.8** (LSP support inside
  `.fitzv`), **11.9** (pedagogic docs).
...
**Sub-paso 11.6.e PARTIAL (§9.z + §9.aa + §9.bb, 2026-07-16)** —
tres passes del día. §9.z shipped SSR emitter `payload` scope en
event bodies + enriched module-not-found hint for
`fitz_liveviews` (targeted `hint:` block con canonical git dep
snippet en `evaluator::load_module` y `codegen::ModuleLoader::
load_module`). §9.aa shipped event-body widening entera —
`emit_event_fn` dispatcher `is_trivial_event_body` → trivial /
widened; wide path prime shadow locals `let <field> = state.
<field>` + walker recursivo `lower_event_body_stmts` acepta
`Stmt::Assign` a Ident (new local o shadow mutation) +
`Stmt::Expr(Expr::If, _)` guards con arm scope truncation;
walker `format_fitz_expr_scoped` widened para `Expr::If`
(single-expr arms via `format_if_arm_value`) + `Expr::StructLit`
(walk fields verbatim). Trivial-path regression zero. Kanban's
`card_editor_save` (`let new_text = if(payload.has("text"))
{payload["text"]} else {text}`) + chat's `send_message` (nested
`if(payload.has("author")){if(payload.has("text")){last_msg =
payload["text"]}}`) `.fitzv` migrations ahora emit + round-trip
cleanly. **§9.bb shipped cross-module `@live_component` auto-
inject** — extends v0.20.1's implicit `flv_register(...)` a
components declarados en imported `.fitzv`/`.fitz` sibling
modules. Nuevo `pub struct ImportedLiveComponent` + field
`TypeEnv.imported_live_components` con `add_imported_live_
components`/`imported_live_components()` accessors + `pub fn
extract_live_components_from_program(program, module_name)`
walker (silent-drop-on-missing-render_for + deterministic sort);
`inject_live_component_registrations` extended con imported loop
(local-wins-over-imported silent skip, bare-Ident emission,
missing-name error con hint `Add \`from <module> import
<TypeName>, <TypeName>_render, ...\``); nuevo `pre_scan_imported_
live_components` en main.rs paralelo bit-a-bit a
`pre_scan_imported_background_fns` (B10) +
`pre_scan_imported_auth_provider` (W12) wired en
`check_program_with_pyi_stubs_and_deps`. +9 unit tests +9bb + 2
E2E `phase_11_6_e_bb_cross_module_*` con stub
`fitz_liveviews.fitz`. **v0.21.0 release** (2026-07-16) — aggregates
Phase 11 entera (11.1 → 11.5 + 11.6.a/b/c/d + 11.6.e §9.z/aa/bb).
See CHANGELOG. **Debt residual (NO bloquea)**: cross-file
`<Child />` composition (§9.y, low priority — ninguno de los 4
ejemplos lo necesita), `fitz check` inject-time errors (checker
...
## v0.20.1 (2026-07-13) — Implicit `flv_register(...)` para LiveView components (fitz-liveviews Phase 5, A.1)

Primer sub-paso de la Fase 5 diferida post-Phase-4 de
`fitz-liveviews`. El compilador auto-genera una llamada
`flv_register("name", InitialState {}, render_fn,
{"event": handler})` por cada tipo con `@live_component`,
consumiendo la metadata que `resolve_program` ya persiste
en `TypeEnv` (`live_components`, `render_handlers`,
`event_handlers`). Sin breaking: la API pública de
`fitz-liveviews` es idéntica; los programas con manual
`flv_register(...)` siguen funcionando y toman precedencia
sobre la inyección implícita.

**Nueva helper** en `src/types.rs`:

- `pub fn inject_live_component_registrations(program:
  &mut Program, env: &TypeEnv) ->
  Result<(), Vec<FitzError>>` — walker determinístico
  (componentes ordenados por nombre) que appendéa un
  `Stmt::Expr(Call flv_register(...))` sintético por
  cada `@live_component`. Validaciones en tiempo de
  inyección:
  - `@live_component` sin `@render_for` matching → error
    claro citando el fn que falta.
  - Field sin `default` en un `@live_component` type →
    error citando el field (el `TypeName {}` sintético
    requiere defaults en todos los fields).
  - `flv_register` no está en scope (ni por `from
    fitz_liveviews import flv_register` ni por `fn
    flv_register` local en tests) → error sugiriendo el
    import canónico.
  - User ya llamó `flv_register("name", ...)` manualmente
    → skip para ese componente (idempotencia sin
    conflicto, respeta intent explícito).
  - Sin `@live_component` en el programa → no-op (return
    Ok early).

**Wiring en el pipeline**: `run_file`, `build_file` y
`build_file_with_bundle` en `src/main.rs` llaman la
helper después de `check_program_with_pyi_stubs` y
antes de eval/codegen. Los errores de inyección abortan
(exit code 1) con el mismo formato que los errores del
checker. `fitz check` no llama la helper — el chequeo es
solo de build/run time.

**Smoke real**: `examples/kanban/` y `examples/dashboard/`
de `fitz-liveviews` compilan (`fitz check`) y corren
(`fitz run`) sin el `flv_register(...)` manual que
exigía Phase 4. El dashboard, con smoke `curl` sobre
`/`, sirve HTML con `data-flv-component-name="metric_tile"`
proving auto-registration end-to-end.

**9 unit tests nuevos** en `src/types.rs::tests::
implicit_register_*`: inyección básica, no-op sin
componentes, skip cuando manual call existe, missing
`@render_for` error, field sin default error, missing
`flv_register` en scope error, alias de import trata
como out-of-scope, orden alfabético, componente sin
events emite Map vacío.

**Deudas residuales derivadas** (NO bloquean uso real):

- Cross-module `@live_component` NO soportado en este
  MVP — solo componentes declarados en el program
  top-level. Paralelo a `imported_auth_provider` /
  `imported_background_fns` (W12, B10) — refinable
  cuando aparezca demanda real.
- `from fitz_liveviews import flv_register as register`
  deja `flv_register` fuera de scope canónico → error
  claro con fix. Los usuarios simplemente no aliasan
  el nombre.
- Custom initial state con valores distintos de los
  defaults del type requiere manual `flv_register(...)`
  explícito — el implicit sintetiza `TypeName {}` que
  usa exclusivamente defaults.

**Próximo norte tras v0.20.1**: los otros dos items de
Phase 5 de `fitz-liveviews` (per-instance init +
`dispatch_to_all`) son features **nuevas** — se deciden
con datos empíricos de un showcase post-cierre. Sin
presión al cierre.
