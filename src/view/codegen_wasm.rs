...
pub fn emit_module_with_components(
    ...
    let mut out = String::new();
    // Compute the merged component tree first (pure data — emits nothing) so
    // the header can gate the `AtomicBool` import on whether any component in
    // the WHOLE tree (local + reachable imported `<Child />`) carries a style
    // block; an imported child with `<style scoped>` is emitted inline here.
...
fn emit_module_header(out: &mut String, has_style: bool) {
    out.push_str(
    ...
    // The `AtomicBool`/`Ordering` import is used ONLY by the per-component
    // `<style>` injection helper (`emit_style_helper` — both `scoped` and
    // `global` styles emit the `static INJECTED: AtomicBool` guard). A crate
    // whose components carry no style block at all would otherwise emit this
    // import unused → an `unused_imports` warning under `wasm-pack build`
    // (the WASM lib.rs crate-level allow covers only non_snake_case +
    // unused_parens). Gate on "any component has a style block", NOT
    // scoped-only, so a `<style global>`-only crate keeps the import.
    if has_style {
...
/// `true` when any component in `file` carries a `<style>` block (scoped OR
/// global). Gates the `AtomicBool`/`Ordering` import in the module header:
/// that import is used only by the per-component `<style>` injection helper
/// (`emit_style_helper`), so a style-less crate can drop it and compile
/// warning-free. Runs over the merged tree (see `emit_module_with_components`)
/// so an imported `<Child />` with a style block keeps the import.
fn any_component_has_style(file: &ExpandedViewFile) -> bool {
...
component Row {
  ...
  <template><input @input="on_type" value="{s}" /></template>
    ...
    #[test]
    fn atomic_import_gated_on_component_style() {
        // The `AtomicBool`/`Ordering` import is used only by the per-component
        // `<style>` injection helper. A component WITH a style block emits it;
        // a style-less component drops it (warning-free WASM crate).
        let styled = emit_module(&parse_expand(counter_shape_src())).unwrap();
        assert!(
            styled.contains("use std::sync::atomic::{AtomicBool, Ordering};"),
            "styled component must keep the atomic import:\n{styled}"
        );

        let style_less = r#"component Bare {
  state { count: Int = 0 }
  event bump() { count = 42 }

  <template>
    <div><span>{count}</span><button @click="bump">+</button></div>
  </template>
}
"#;
        let out = emit_module(&parse_expand(style_less)).unwrap();
        assert!(
            !out.contains("AtomicBool"),
            "style-less component must NOT emit the atomic import:\n{out}"
        );
        // Sanity: the style helper (the only consumer) is also absent.
        assert!(
            !out.contains("static INJECTED: AtomicBool"),
            "no style helper without a style block:\n{out}"
        );
    }
