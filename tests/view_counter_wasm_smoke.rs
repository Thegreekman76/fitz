...
/// Load `Counter.fitzv`, run parse → expand → check → emit_module,
/// and return the emitted Rust source with the composed entry
/// point (`#[wasm_bindgen(start)] pub fn start()`) appended.
fn generate_lib_rs_from_fitzv() -> String {
    ...
    let emitted = fitz::view::emit_module(&expanded)
        .unwrap_or_else(|e| panic!("view::emit_module failed on Counter.fitzv:\n{}", e));

    // Compose the entry point. `emit_module` deliberately does
    // NOT emit `#[wasm_bindgen(start)]` — the 11.5 CLI will
    // eventually compose this per-target. For the 11.4.c POC
    // we compose it here.
    let entry = "\n\n\
        ...
        \x20\x20\x20\x20let counter = Counter::new();\n\
        \x20\x20\x20\x20counter.mount(\"#app\")?;\n\
        \x20\x20\x20\x20Ok(())\n\
        }\n";

    let mut out = String::new();
    out.push_str("// AUTO-GENERATED by `tests/view_counter_wasm_smoke.rs`.\n");
    out.push_str("// Do NOT edit by hand — regenerate via:\n");
    out.push_str("//\n");
    out.push_str("//   cargo test --test view_counter_wasm_smoke\n");
    out.push_str("//\n");
    out.push_str("// The source of truth is `../Counter.fitzv`.\n\n");
...
fn regenerate_counter_lib_rs() {
    ...
    let checks: &[(&str, &str)] = &[
        ...
        ("counter.mount(\"#app\")", "entry point mounts on `#app`"),
