// AUTO-GENERATED by `fitz build --target wasm-client`.
// Do NOT edit by hand — regenerate from the source `.fitzv`.
//
// Source: App.fitzv
//
// Composed by `src/view/wasm_build.rs::compose_lib_rs` (Phase 11.5.c).
// The tail below is the `#[wasm_bindgen(start)]` wrapper the CLI
// appends to the emitter output; the emitter itself does NOT emit
// the entry point so future targets (SSR, hybrid) can compose
// their own.

// Generated by fitz view WASM emitter (Fase 11.4.b).
// Do NOT edit — regenerate from the source `.fitzv`.

// Phase 11.5.e (§9.o Debt residual) — crate-level allows for
// the two cosmetic warnings that are structural to the
// emitter output, not correctness bugs:
//   - `non_snake_case`: component names are PascalCase by
//     convention (11.5.d), and the synthesised style helper
//     `__inject_style_<Component>_<scope>` reflects that.
//   - `unused_parens`: BinOp lowering wraps sub-expressions
//     in `(...)` to preserve precedence when nested. When
//     the BinOp is the entire RHS of an assignment the outer
//     parens are redundant but harmless.
#![allow(non_snake_case, unused_parens)]

use std::cell::RefCell;
use std::rc::Rc;
use std::sync::atomic::{AtomicBool, Ordering};
use wasm_bindgen::prelude::*;
use wasm_bindgen::JsCast;
use web_sys::{Event, HtmlElement};

// Phase 11.12 — hydration cursor helpers. Advance a sibling cursor to
// the next element / text node so the adopt walk maps template nodes
// onto the server-painted DOM in DFS order without re-creating them.
fn __flv_next_element(__cursor: &mut Option<web_sys::Node>) -> Option<web_sys::Element> {
    while let Some(__n) = __cursor.clone() {
        *__cursor = __n.next_sibling();
        if let Some(__el) = __n.dyn_ref::<web_sys::Element>() {
            return Some(__el.clone());
        }
    }
    None
}
fn __flv_next_text(__cursor: &mut Option<web_sys::Node>) -> Option<web_sys::Text> {
    while let Some(__n) = __cursor.clone() {
        *__cursor = __n.next_sibling();
        if let Some(__t) = __n.dyn_ref::<web_sys::Text>() {
            return Some(__t.clone());
        }
    }
    None
}

pub struct App {
    title: RefCell<String>,
    likes: RefCell<i64>,
    __child_slot_0: RefCell<Option<Rc<Card>>>,
    root: RefCell<Option<HtmlElement>>,
}

impl App {
    pub fn new() -> Rc<Self> {
        Rc::new(App {
            title: RefCell::new("default-title".to_string()),
            likes: RefCell::new(0i64),
            __child_slot_0: RefCell::new(None),
            root: RefCell::new(None),
        })
    }

    fn like(self: &Rc<Self>) {
        let __rhs = ((*self.likes.borrow()) + 1i64);
        *self.likes.borrow_mut() = __rhs;
        self.render();
    }

    pub fn mount(self: &Rc<Self>, selector: &str) -> Result<(), JsValue> {
        let document = web_sys::window().unwrap().document().unwrap();
        let root = document
            .query_selector(selector)?
            .ok_or_else(|| JsValue::from_str("mount: selector matched no element"))?
            .dyn_into::<HtmlElement>()?;
        self.mount_into(root)
    }

    pub fn mount_into(self: &Rc<Self>, root: HtmlElement) -> Result<(), JsValue> {
        *self.root.borrow_mut() = Some(root);
        self.render();
        Ok(())
    }

    fn render(self: &Rc<Self>) {
        let root_ref = self.root.borrow();
        let root = match root_ref.as_ref() {
            Some(r) => r,
            None => return,
        };
        while let Some(child) = root.first_child() {
            let _ = root.remove_child(&child);
        }
        let document = web_sys::window().unwrap().document().unwrap();
        let __el0 = document.create_element("div").unwrap();
        __el0.set_attribute("class", "page").unwrap();
        let __el1 = document.create_element("h1").unwrap();
        let __t2 = document.create_text_node("SSR to client hydration: composition");
        __el1.append_child(&__t2).unwrap();
        __el0.append_child(&__el1).unwrap();
        let __el3 = document.create_element("div").unwrap();
        __el3.set_attribute("class", "__fitz-child-Card").unwrap();
        __el0.append_child(&__el3).unwrap();
        let __child4 = {
            let mut __slot = self.__child_slot_0.borrow_mut();
            if __slot.is_none() { *__slot = Some(Card::new()); }
            __slot.as_ref().unwrap().clone()
        };
        {
            let __parent = self.clone();
            *__child4.__slot.borrow_mut() = Some(Rc::new(move |__t: &web_sys::Node| __parent.__render_slot_0(__t)));
        }
        let __el3_html = __el3.clone().dyn_into::<HtmlElement>().unwrap();
        __child4.mount_into(__el3_html).unwrap();
        root.append_child(&__el0).unwrap();
    }
    fn __render_slot_0(self: &Rc<Self>, __target: &web_sys::Node) {
        let document = web_sys::window().unwrap().document().unwrap();
        let __el0 = document.create_element("p").unwrap();
        __el0.set_attribute("class", "lead").unwrap();
        let __t1 = document.create_text_node("Slot content, rendered in the PARENT scope.");
        __el0.append_child(&__t1).unwrap();
        __target.append_child(&__el0).unwrap();
        let __el2 = document.create_element("p").unwrap();
        __el2.set_attribute("class", "ttl-row").unwrap();
        let __t3 = document.create_text_node("title: ");
        __el2.append_child(&__t3).unwrap();
        let __el4 = document.create_element("span").unwrap();
        __el4.set_attribute("class", "ttl").unwrap();
        let __interp5 = format!("{}", (*self.title.borrow()));
        let __t6 = document.create_text_node(&__interp5);
        __el4.append_child(&__t6).unwrap();
        __el2.append_child(&__el4).unwrap();
        __target.append_child(&__el2).unwrap();
        let __el7 = document.create_element("button").unwrap();
        __el7.set_attribute("class", "like").unwrap();
        {
            let __self_clone = self.clone();
            let __closure = Closure::wrap(Box::new(move |_evt: Event| {
                App::like(&__self_clone);
            }) as Box<dyn FnMut(Event)>);
            __el7.add_event_listener_with_callback("click", __closure.as_ref().unchecked_ref()).unwrap();
            __closure.forget();
        }
        let __t8 = document.create_text_node("like");
        __el7.append_child(&__t8).unwrap();
        __target.append_child(&__el7).unwrap();
        let __el9 = document.create_element("p").unwrap();
        __el9.set_attribute("class", "likes-row").unwrap();
        let __t10 = document.create_text_node("likes: ");
        __el9.append_child(&__t10).unwrap();
        let __el11 = document.create_element("span").unwrap();
        __el11.set_attribute("class", "ln").unwrap();
        let __interp12 = format!("{}", (*self.likes.borrow()));
        let __t13 = document.create_text_node(&__interp12);
        __el11.append_child(&__t13).unwrap();
        __el9.append_child(&__el11).unwrap();
        __target.append_child(&__el9).unwrap();
    }

    fn __apply_state_json(self: &Rc<Self>, __json: &str) {
        let __v: serde_json::Value = match serde_json::from_str(__json) { Ok(__j) => __j, Err(_) => return, };
        let _ = (&__v, self);
        if let Some(__x) = __v.get("title").and_then(|__j| __j.as_str()) { *self.title.borrow_mut() = __x.to_string(); }
        if let Some(__x) = __v.get("likes").and_then(|__j| __j.as_i64()) { *self.likes.borrow_mut() = __x; }
    }

    pub fn hydrate(self: &Rc<Self>, root: HtmlElement) -> Result<(), JsValue> {
        if let Some(__sel) = web_sys::window().unwrap().document().unwrap().get_element_by_id("__flv_state_App") {
            if let Some(__txt) = __sel.text_content() { self.__apply_state_json(&__txt); }
        }
        let mut __cur_root = root.first_child();
        *self.root.borrow_mut() = Some(root);
        if let Some(__hel0) = __flv_next_element(&mut __cur_root) {
        let mut __hcur1 = __hel0.first_child();
        if let Some(__hel2) = __flv_next_element(&mut __hcur1) {
        let mut __hcur3 = __hel2.first_child();
        let _ = __flv_next_text(&mut __hcur3);
        }
        if let Some(__hchild4) = __flv_next_element(&mut __hcur1) {
        let __child5 = {
            let mut __slot = self.__child_slot_0.borrow_mut();
            if __slot.is_none() { *__slot = Some(Card::new()); }
            __slot.as_ref().unwrap().clone()
        };
        {
            let __parent = self.clone();
            *__child5.__slot.borrow_mut() = Some(Rc::new(move |__t: &web_sys::Node| __parent.__render_slot_0(__t)));
        }
        {
            let __parent = self.clone();
            *__child5.__hslot.borrow_mut() = Some(Rc::new(move |__c: &mut Option<web_sys::Node>| __parent.__hydrate_slot_0(__c)));
        }
        let __hchild4_html = __hchild4.clone().dyn_into::<HtmlElement>().unwrap();
        __child5.hydrate(__hchild4_html).unwrap();
        }
        }
        Ok(())
    }

    fn __hydrate_slot_0(self: &Rc<Self>, __cursor: &mut Option<web_sys::Node>) {
        if let Some(__hel0) = __flv_next_element(&mut (*__cursor)) {
        let mut __hcur1 = __hel0.first_child();
        let _ = __flv_next_text(&mut __hcur1);
        }
        if let Some(__hel2) = __flv_next_element(&mut (*__cursor)) {
        let mut __hcur3 = __hel2.first_child();
        let _ = __flv_next_text(&mut __hcur3);
        if let Some(__hel4) = __flv_next_element(&mut __hcur3) {
        let mut __hcur5 = __hel4.first_child();
        let _ = __flv_next_text(&mut __hcur5);
        }
        }
        if let Some(__hel6) = __flv_next_element(&mut (*__cursor)) {
        {
            let __self_clone = self.clone();
            let __closure = Closure::wrap(Box::new(move |_evt: Event| {
                App::like(&__self_clone);
            }) as Box<dyn FnMut(Event)>);
            __hel6.add_event_listener_with_callback("click", __closure.as_ref().unchecked_ref()).unwrap();
            __closure.forget();
        }
        let mut __hcur7 = __hel6.first_child();
        let _ = __flv_next_text(&mut __hcur7);
        }
        if let Some(__hel8) = __flv_next_element(&mut (*__cursor)) {
        let mut __hcur9 = __hel8.first_child();
        let _ = __flv_next_text(&mut __hcur9);
        if let Some(__hel10) = __flv_next_element(&mut __hcur9) {
        let mut __hcur11 = __hel10.first_child();
        let _ = __flv_next_text(&mut __hcur11);
        }
        }
    }

}

pub struct Card {
    taps: RefCell<i64>,
    __slot: RefCell<Option<Rc<dyn Fn(&web_sys::Node)>>>,
    __hslot: RefCell<Option<Rc<dyn Fn(&mut Option<web_sys::Node>)>>>,
    root: RefCell<Option<HtmlElement>>,
}

impl Card {
    pub fn new() -> Rc<Self> {
        Rc::new(Card {
            taps: RefCell::new(0i64),
            __slot: RefCell::new(None),
            __hslot: RefCell::new(None),
            root: RefCell::new(None),
        })
    }

    fn tap(self: &Rc<Self>) {
        let __rhs = ((*self.taps.borrow()) + 1i64);
        *self.taps.borrow_mut() = __rhs;
        self.render();
    }

    pub fn mount(self: &Rc<Self>, selector: &str) -> Result<(), JsValue> {
        let document = web_sys::window().unwrap().document().unwrap();
        let root = document
            .query_selector(selector)?
            .ok_or_else(|| JsValue::from_str("mount: selector matched no element"))?
            .dyn_into::<HtmlElement>()?;
        self.mount_into(root)
    }

    pub fn mount_into(self: &Rc<Self>, root: HtmlElement) -> Result<(), JsValue> {
        *self.root.borrow_mut() = Some(root);
        self.render();
        Ok(())
    }

    fn render(self: &Rc<Self>) {
        let root_ref = self.root.borrow();
        let root = match root_ref.as_ref() {
            Some(r) => r,
            None => return,
        };
        while let Some(child) = root.first_child() {
            let _ = root.remove_child(&child);
        }
        let document = web_sys::window().unwrap().document().unwrap();
        let __el0 = document.create_element("section").unwrap();
        __el0.set_attribute("class", "card").unwrap();
        let __el1 = document.create_element("header").unwrap();
        __el1.set_attribute("class", "card-head").unwrap();
        let __t2 = document.create_text_node("Card header (from the child)");
        __el1.append_child(&__t2).unwrap();
        __el0.append_child(&__el1).unwrap();
        let __el3 = document.create_element("div").unwrap();
        __el3.set_attribute("class", "card-body").unwrap();
        if let Some(__cb) = self.__slot.borrow().as_ref() {
            let __slot_target: &web_sys::Node = __el3.as_ref();
            __cb(__slot_target);
        } else {
        let __el4 = document.create_element("em").unwrap();
        let __t5 = document.create_text_node("default fallback content");
        __el4.append_child(&__t5).unwrap();
        __el3.append_child(&__el4).unwrap();
        }
        __el0.append_child(&__el3).unwrap();
        let __el6 = document.create_element("p").unwrap();
        __el6.set_attribute("class", "tap-row").unwrap();
        let __t7 = document.create_text_node("child taps: ");
        __el6.append_child(&__t7).unwrap();
        let __el8 = document.create_element("span").unwrap();
        __el8.set_attribute("class", "tn").unwrap();
        let __interp9 = format!("{}", (*self.taps.borrow()));
        let __t10 = document.create_text_node(&__interp9);
        __el8.append_child(&__t10).unwrap();
        __el6.append_child(&__el8).unwrap();
        __el0.append_child(&__el6).unwrap();
        let __el11 = document.create_element("button").unwrap();
        __el11.set_attribute("class", "tap").unwrap();
        {
            let __self_clone = self.clone();
            let __closure = Closure::wrap(Box::new(move |_evt: Event| {
                Card::tap(&__self_clone);
            }) as Box<dyn FnMut(Event)>);
            __el11.add_event_listener_with_callback("click", __closure.as_ref().unchecked_ref()).unwrap();
            __closure.forget();
        }
        let __t12 = document.create_text_node("tap");
        __el11.append_child(&__t12).unwrap();
        __el0.append_child(&__el11).unwrap();
        root.append_child(&__el0).unwrap();
    }
    fn __apply_state_json(self: &Rc<Self>, __json: &str) {
        let __v: serde_json::Value = match serde_json::from_str(__json) { Ok(__j) => __j, Err(_) => return, };
        let _ = (&__v, self);
        if let Some(__x) = __v.get("taps").and_then(|__j| __j.as_i64()) { *self.taps.borrow_mut() = __x; }
    }

    pub fn hydrate(self: &Rc<Self>, root: HtmlElement) -> Result<(), JsValue> {
        if let Some(__sel) = web_sys::window().unwrap().document().unwrap().get_element_by_id("__flv_state_Card") {
            if let Some(__txt) = __sel.text_content() { self.__apply_state_json(&__txt); }
        }
        let mut __cur_root = root.first_child();
        *self.root.borrow_mut() = Some(root);
        if let Some(__hel0) = __flv_next_element(&mut __cur_root) {
        let mut __hcur1 = __hel0.first_child();
        if let Some(__hel2) = __flv_next_element(&mut __hcur1) {
        let mut __hcur3 = __hel2.first_child();
        let _ = __flv_next_text(&mut __hcur3);
        }
        if let Some(__hel4) = __flv_next_element(&mut __hcur1) {
        let mut __hcur5 = __hel4.first_child();
        let __hcb = self.__hslot.borrow().clone();
        if let Some(__hcb) = __hcb {
            __hcb(&mut __hcur5);
        } else {
        if let Some(__hel6) = __flv_next_element(&mut __hcur5) {
        let mut __hcur7 = __hel6.first_child();
        let _ = __flv_next_text(&mut __hcur7);
        }
        }
        }
        if let Some(__hel8) = __flv_next_element(&mut __hcur1) {
        let mut __hcur9 = __hel8.first_child();
        let _ = __flv_next_text(&mut __hcur9);
        if let Some(__hel10) = __flv_next_element(&mut __hcur9) {
        let mut __hcur11 = __hel10.first_child();
        let _ = __flv_next_text(&mut __hcur11);
        }
        }
        if let Some(__hel12) = __flv_next_element(&mut __hcur1) {
        {
            let __self_clone = self.clone();
            let __closure = Closure::wrap(Box::new(move |_evt: Event| {
                Card::tap(&__self_clone);
            }) as Box<dyn FnMut(Event)>);
            __hel12.add_event_listener_with_callback("click", __closure.as_ref().unchecked_ref()).unwrap();
            __closure.forget();
        }
        let mut __hcur13 = __hel12.first_child();
        let _ = __flv_next_text(&mut __hcur13);
        }
        }
        Ok(())
    }

}



// Composed entry point — NOT part of emit_module output.
// `fitz build --target wasm-client` appends this wrapper. Phase
// 11.12: hydrate the server-painted DOM when the mount root
// already has content, else fresh-mount into an empty root.
#[wasm_bindgen(start)]
pub fn start() -> Result<(), JsValue> {
    console_error_panic_hook::set_once();
    let root = App::new();
    let __document = web_sys::window().unwrap().document().unwrap();
    if let Some(__mount) = __document.query_selector("#app")? {
        if __mount.first_element_child().is_some() {
            let __el = __mount.dyn_into::<web_sys::HtmlElement>()?;
            return root.hydrate(__el);
        }
    }
    root.mount("#app")?;
    Ok(())
}
